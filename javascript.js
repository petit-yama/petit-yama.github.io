...
function draw(maxTime, timebar_len) {
    ...
    fillRoundedRect(ctx, 7, 7, (366 / maxTime) * timebar_len, 46, 5)
    ...
    ctx.fillText("残り " + countTxt, 80, 45);
}
...
function NotificationButton() {
    Notification.requestPermission().then((result) => {
        ...
        switch (result) {
            case "default":
                document.getElementById("notification_button").textContent = "デスクトップ通知する？"
                break;
            case "denied":
                document.getElementById("notification_button").textContent = "デスクトップ通知 不可"
                break;
            case "granted":
                document.getElementById("notification_button").textContent = "デスクトップ通知やめる？"
                break;
        }
    })
};*/
...
function addressChangeWatchToIframe(listYoutubeAddress) {
    ...
    if (listYoutubeAddress.indexOf("https://www.youtube.com/watch?", 1)) {
        // 「https://www.youtube.com/watch?v=」が含まれる場合は通常のyoutubeURLとみなす
        // 短縮URLは2025/8月終了ということで非対応
